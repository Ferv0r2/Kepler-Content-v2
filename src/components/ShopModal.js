...
const ShopModal = ({ open, close, tokenId1, tokenId2, tokenId3, urls, tx }) => {
  const num = [tokenId1, tokenId2, tokenId3];
              ...
              <div className="pb-8 text-center items-center">
                ...
                <div className="relative flex w-full sm:w-1/2 m-auto flex-wrap">
                  {urls ? (
                    urls.map((v, i) => {
                      return (
                        <div className="w-5/12 m-auto" key={v}>
                          <img className="m-2" src={urls[i]} />
                          <p>{`#${num[i]}`}</p>
                        </div>
                      );
                    })
